...
namespace Kernel {
    void KernelMain() {
        ...
        // Processes::Scheduler::the().CreateProcessFromElf(test_a, size_of_testa_page_aligned, "testa");
        Processes::Scheduler::the().CreateProcessFromElf(test_b, size_of_testb_page_aligned, "testb");

        // Free the tempory memcopy stuff
        VM::Manager::the().FreePages(test_a);
        VM::Manager::the().FreePages(test_b);
