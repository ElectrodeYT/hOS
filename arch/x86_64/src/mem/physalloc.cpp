...
void* __physmem_allocate_page(int count = 1) {
    ...
    while(curr != NULL) {
        ...
        for(int page = 0; page < (curr->size / 4096); page++) {
            ...
            if((curr->bitmap[byte] >> bit) & 1) {
                ...
                bool enough = true;
                int page_failed = 0;
                for(int check_page = page; check_page < (page + count); check_page++) {
                    // Really fancy shit
                ...
                for(int page_set = page; page_set < (page + count); page_set++) {
        ...
        if(curr->base <= (uint64_t)page && (curr->base + curr->size) > (uint64_t)page) {
            ...
            if(curr->bitmap != NULL) {
                for(int i = 0; i < count; i++) {
                    int page = (((uint64_t)(page + i) - curr->base) / 4096);
                    curr->bitmap[(page + i) / 8] &= ~(1 << ((page + i) % 8));
                }
            }
            return;
        }
